...
# Then A_star to x = 1 will be supersonic sln.
# Stop at 1 = x. need to apply normal shock
for x, area in zip(df['x_location'], df['area']):
    ...
    if x <= x_star:
        ...
        # normal shock exactly at x=1
        shock_row = df.loc[df['x_location'] == 0.75]
        A_shock = float(shock_row['area'])
        A_ratio_pre = (A_shock / A_star) ** 2
        M1 = inverse_area_mach_relation(A_ratio_pre, gamma)[1]
        M2 = M2_from_normal_shock(M1, gamma)

        # find new stag pressure to normalize by
        P01_P1 = P0_P(M1, gamma)
        P1_P2 = 1/ P2_P1_from_normal_shock(M1, gamma)
        P2_P02 = 1/ P0_P(M2, gamma)
        P01_P02 = P01_P1 * P1_P2 * P2_P02

        M = M2
        P_P0 = 1 / P0_P(M2, gamma) / P01_P02


    else:
        # subsonic after shock bc we expand back up (not used here. whatever)
        M = inverse_area_mach_relation(A_ratio_sq, gamma)[0]
        P_P0 = 1 / P0_P(M, gamma) / P01_P02 # renormalize w new stag pressure after N.S.
...
# Final P/P0: 0.19254883010474497

plt.plot(df['x_location'], df['P_P0'])
plt.xlabel('x location')
plt.ylabel('P/P0')
plt.title('Normalized Pressure Distribution along Nozzle Given N.S. at x=1 (exit)')
plt.grid(True)
plt.show()
