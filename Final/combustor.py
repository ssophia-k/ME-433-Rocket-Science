...
def evaluate_combustor(length, M_in, P_in, T_in, m_dot_air, width, M_out):
    """
    Evaluates combustor exit properties for a FIXED length combustor
    given a specific M_out s.t we can choke correctly in the next sections (converging to nozzle)
    ...
    OUTPUTS THE REQUIRED FUEL MASS FLOW RATE S.T. WE REACH EXACTLY M_OUT
    """
    ...
    # Know M_out requirement. So can get avg values
    M_target = M_out
    ...
    # Get fuel flow required to hit this
    q_added = cp*(T0_target - T0_in)
    m_dot_fuel = (q_added * m_dot_air) / LHV_H2

    # Check if burn time actually matches the time we spend in this tube
    burnt_status = ""
    tau_ms = 325 * (P_avg_atm ** (-1.6)) * np.exp(-0.8 * (T0_avg / 1000.0))
    tau_sec = tau_ms / 1000.0
    time_in_combustor = length / u_avg
